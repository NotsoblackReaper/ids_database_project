...
create table components(
    serial_nr number(10,0) GENERATED BY DEFAULT AS IDENTITY(START with 1 INCREMENT by 1),
    parent_nr number(10,0) null,
...
create table incidentReports(
    guid number(10,0) not null,
    k3 char(12) unique not null,
    date_filed date null,
    severity number (1,0) null,
    primary key(guid),
    CONSTRAINT report_document_fk FOREIGN KEY (guid) REFERENCES documents (guid) on delete cascade
);
...
create table employees(
    ...
    email varchar(45) default 'employee@company.at',
    primary key(employeeId)
);

 create table incidents(
    guid number(10,0) GENERATED ALWAYS AS IDENTITY(START with 1 INCREMENT by 1),
    type varchar(45) null,
    incident_cost number(20,2) default 0,
    primary key(guid),
    constraint incident_cost_check check (incident_cost >= 0)
);
...
create table affects(
    ...
    guid number(10,0) not null,
    primary key(serial_nr,guid),
    CONSTRAINT affects_component_fk FOREIGN KEY (serial_nr) REFERENCES components (serial_nr),
    CONSTRAINT affects_incident_fk FOREIGN KEY (guid) REFERENCES incidents (guid)
);
